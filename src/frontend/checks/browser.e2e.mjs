...
try {
    ...
    await page.setViewportSize({ width: 1440, height: 900 });
    await frames();
    assert.equal(await page.locator('.pane-tool-grid').evaluate(element => getComputedStyle(element).scrollbarWidth), 'auto', 'desktop tools retain their normal non-looping grid');
