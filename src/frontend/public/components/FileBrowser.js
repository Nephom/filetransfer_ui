...
const FileBrowser = ({ token, user }) => {
    ...
    const loadFiles = async (path = currentPath) => {
        ...
        try {
            ...
            setDisplayPath(data.currentPath || 'Home');
    ...
    const remove = async () => {
        ...
        try {
            ...
            if (!response.ok) { const data = await response.json(); throw new Error(data.error || 'Delete failed.'); } loadFiles(currentPath);
        } catch (requestError) { setError(requestError.message); }
    };
    const saveFolder = async (event) => {
        ...
        try {
            ...
            if (!response.ok) { const data = await response.json(); throw new Error(data.error || 'Could not create folder.'); } setModal(null); loadFiles(currentPath);
        } catch (requestError) { setError(requestError.message); }
    };
    const saveRename = async (event) => {
        ...
        try {
            ...
            if (!response.ok) { const data = await response.json(); throw new Error(data.error || 'Rename failed.'); } setModal(null); loadFiles(currentPath);
        } catch (requestError) { setError(requestError.message); }
    };
    const upload = async (event) => {
        ...
        try { const response = await fetch('/api/upload/multiple', { method: 'POST', headers: authHeaders, body: data }); if (!response.ok) throw new Error('Upload failed.'); loadFiles(currentPath); }
    ...
    return <div className="explorer" onContextMenu={(event) => event.preventDefault()}>
        <header className="titlebar"><span className="app-mark" /><span className="app-name">File Transfer</span><span className="account">{user.username}</span></header>
        <nav className="commandbar">
            ...
            <button disabled={!selectedItems.length} onClick={download}>Download</button><button disabled={selectedItems.length !== 1} onClick={() => setModal('rename')}>Rename</button>
        ...
        <div className="navigation"><button className="nav-button" aria-label="Go up" disabled={!currentPath && !searching} onClick={goUp}>↑</button><div className="crumbs"><button onClick={() => loadFiles('')}>Home</button>{crumbs.map((part, index) => <React.Fragment key={`${part}-${index}`}><span className="crumb-separator">›</span><button onClick={() => loadFiles(crumbs.slice(0, index + 1).join('/'))}>{part}</button></React.Fragment>)}</div><input className="search" value={search} onChange={(event) => setSearch(event.target.value)} onKeyDown={(event) => event.key === 'Enter' && searchFiles()} placeholder="Search files" aria-label="Search files" /></div>
        <main className="workspace"><aside className="sidebar"><span className="sidebar-label">Locations</span><button className="side-item active" onClick={() => loadFiles('')}><span className="folder-mini" />Home</button><span className="sidebar-label">This folder</span><button className="side-item" onClick={() => loadFiles(currentPath)}><span className="folder-mini" />Refresh view</button></aside>
            <section className="content"><div className="content-heading"><h1>{displayPath}</h1>{selectedItems.length > 0 && <span className="selection-count">{selectedItems.length} selected</span>}</div>{error && <div className="notice">{error}</div>}
                <div className="file-area">{loading ? <div className="empty"><strong>Loading files...</strong></div> : files.length === 0 ? <div className="empty"><strong>{searching ? 'No matching files' : 'This folder is empty'}</strong><span>{searching ? 'Try a different search term.' : 'Upload files or create a folder to get started.'}</span></div> : <table className="file-table"><thead><tr><th>Name</th><th>Date modified</th><th>Type</th><th>Size</th></tr></thead><tbody>{files.map((file) => <tr key={itemKey(file)} tabIndex="0" className={`file-row ${selected.includes(itemKey(file)) ? 'selected' : ''}`} onClick={(event) => choose(file, event)} onDoubleClick={() => file.isDirectory ? openFolder(file) : download([file])} onContextMenu={(event) => openContext(event, file)}><td><span className="file-name-cell"><span className="file-icon">{fileIcon(file)}</span>{file.name}</span></td><td className="muted">{formatDate(file.modified || file.modifiedTime)}</td><td className="muted">{file.isDirectory ? 'File folder' : (file.type || 'File')}</td><td className="muted">{file.isDirectory ? '--' : formatSize(file.size)}</td></tr>)}</tbody></table>}</div>
            </section></main>
        <footer className="statusbar"><span>{files.length} item{files.length === 1 ? '' : 's'}</span><span>{searching ? 'Search results' : currentPath || 'Home'}</span></footer>
        {context && <div className="context-menu" style={{ left: context.x, top: context.y }} onClick={(event) => event.stopPropagation()}><button onClick={() => action(download)}>Download</button><button disabled={selectedItems.length !== 1} onClick={() => action(() => setModal('rename'))}>Rename</button><button disabled={selectedItems.length !== 1 || selectedItems[0].isDirectory} onClick={() => action(() => { setShareLink(''); setModal('share'); })}>Share</button><hr /><button onClick={() => action(remove)}>Delete</button></div>}
        {modal === 'folder' && <Dialog title="New folder" onClose={() => setModal(null)}><form onSubmit={saveFolder}><p>Create a folder in {currentPath || 'Home'}.</p><label>Folder name<input name="folderName" autoFocus required /></label><DialogActions onClose={() => setModal(null)} label="Create" /></form></Dialog>}
