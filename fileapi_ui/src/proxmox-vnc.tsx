...
export function ProxmoxVncWorkspace({ workspaceName, entries, activeEntryId, secrets, onSelectEntry, onChangeEntries, onChangeSecret }: Props) {
  ...
  useEffect(() => { setPassword(entry ? secrets[entry.id]?.password || "" : ""); setError(""); setStatus("Not connected"); }, [entry?.id, secrets]);
  ...
  const connect = async () => {
    ...
    try {
      ...
      rfb.addEventListener("securityfailure", (event: Event) => setError(String((event as CustomEvent).detail || "VNC security failure")));
      rfb.addEventListener("credentialsrequired", () => rfb.sendCredentials({ password: connection.password }));
      rfbRef.current = rfb;
    } catch (value) { setStatus("Connection failed"); setError(value instanceof Error ? value.message : String(value)); }
    finally { setLoading(false); }
  };
  const disconnect = () => { rfbRef.current?.disconnect(); rfbRef.current = null; setStatus("Disconnected"); };
  ...
  return <div className="vnc-workspace"><VncEntries entries={entries} activeEntryId={activeEntryId} onSelectEntry={onSelectEntry} onChangeEntries={onChangeEntries} />
