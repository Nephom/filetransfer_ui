...
export const helpSections: HelpSection[] = [
  {
    id: "getting-started", title: "Getting Started", icon: "compass", pages: [
      { id: "login", title: "Login and server connection", summary: "Connect nFterm to an HTTPS file server.", content: <Page><p>Enter the server address without a protocol, path, or port. The default HTTPS port is <code>9443</code>. nFterm verifies TLS certificates by default. The explicit <strong>Ignore SSL certificate verification</strong> option is only for a trusted private endpoint whose certificate cannot be installed in the operating system trust store.</p><List><li>Use <strong>Sign in</strong> for the API file browser.</li></List></Page> },
      { id: "interface", title: "Interface overview", summary: "Understand the header, panes, toolbar, and status areas.", content: <Page><p>The header contains the current account, remote Location selector, and connection status. The command bar acts on the active pane. In Split mode, the active pane indicator tells you whether actions target LOCAL or REMOTE.</p><p>The file area supports keyboard selection, marquee selection, folder navigation, search, sorting, and details/grid views.</p><p>The SSH Terminal runs in its own bottom layer independently of Location, REST API, and VNC layouts. It can maximize below the header, covering the command bar, navigation, workspace, and status bar. Dragging its resize bar to the maximum height uses the same layout.</p></Page> },
    ],
  },
  {
  ...
  {
    id: "rest-api", title: "REST API", icon: "compass", pages: [
      { id: "redfish-session-auth", title: "Redfish Session Auth", summary: "Log in to HPE iLO Redfish APIs and reuse the session safely.", content: <Page><p>Use this flow for HPE iLO and other Redfish services that provide a <code>SessionService/Sessions</code> endpoint.</p><ol><li>Set <strong>Authentication Mode</strong> to <strong>Session Auth</strong>.</li><li>Enter your Redfish username.</li><li>Enter your Redfish password.</li><li>Click <strong>Use Redfish SessionService preset</strong>.</li><li>Click <strong>Login</strong>. The same button changes to <strong>Logout</strong> after success.</li><li>Confirm that <strong>REST session established for this entry.</strong> appears.</li><li>Click <strong>GET</strong> to read the current Redfish resource.</li><li>Click <strong>Logout</strong> to DELETE the advertised Redfish session and clear local session state.</li></ol><p>The token is kept for this REST API entry and reused when you click a history item, breadcrumb, <code>@odata.id</code>, resource link, Redfish action, or download link. nFterm does not read browser cookies or sessions created by other tools.</p><h3>HPE IML Monitor</h3><p>Start creates a new Desktop CSV session file after the HPE ComputerSystem serial number is discovered. New IML entries are appended immediately. The App display keeps at most 50 entries.</p><p>The monitor retries transient iLO connection failures with backoff. It re-discovers Redfish resources after reconnecting. A manual Stop cancels polling and retry. A complete AHS download appears only after manual Stop. AHS download has no date-range selector and does not filter the response by date.</p></Page> },
      { id: "redfish-navigation", title: "Redfish navigation", summary: "Browse @odata.id links, collections, actions, and downloads.", content: <Page><p>Redfish resources commonly expose navigation through <code>@odata.id</code>, <code>href</code>, <code>Members</code>, and nested <code>Links</code>. Click a resource link to issue a new GET. Use the breadcrumbs or Recent GET paths to revisit a resource.</p><p>Power, BIOS, reset, and other <code>Actions.*.target</code> entries are shown separately and require confirmation before a POST is sent. Download links such as <code>DownloadUri</code> open a LOCAL folder picker and save the response using the current REST session.</p></Page> },
    ],
  },
  {
  ...
  {
    id: "proxmox-vnc", title: "Proxmox VNC", icon: "workspace", pages: [
      { id: "proxmox-entry", title: "Create a Proxmox entry", summary: "Configure HTTPS access to a Proxmox cluster.", content: <Page><p>Open <strong>Workspace Manager</strong>, enable the Proxmox VNC mode in Settings, and add a Proxmox VNC entry.</p><List><li>Enter the Proxmox host and HTTPS API port, normally <code>8006</code>.</li><li>Enter the Proxmox account name and choose the <code>pam</code> or <code>pve</code> realm.</li><li>Save the entry, then enter the account password and select <strong>Login</strong>.</li><li>Use the VM list to select the node and guest. Both QEMU and LXC guests are supported.</li></List><p>Passwords are kept through the desktop credential store when saved. They are not written to workspace JSON or operation logs.</p><p>The Add/Edit Proxmox VNC Entry dialog has three paged sections, selected with the buttons under the title: <strong>Host Entry</strong> (the fields above), <strong>VM SSH</strong>, and <strong>Host SSH (jump)</strong> -- see <strong>File transfer credentials</strong> below.</p></Page> },
      { id: "proxmox-connect", title: "Connect to a VM", summary: "Start and safely close a noVNC connection.", content: <Page><p>After login, select a node and VM, then choose <strong>Connect</strong>. nFterm requests a short-lived Proxmox VNC proxy ticket and creates a loopback WebSocket relay for the embedded noVNC client.</p><List><li>The relay validates an exact connection path and one-time token in addition to the Proxmox ticket.</li><li>Switching to another Proxmox entry, disconnecting, or closing the workspace cancels a pending relay.</li><li>There is no fixed idle timeout while you remain on the same entry.</li><li>Use <strong>View only</strong> when keyboard and mouse input must be disabled.</li><li>Use <strong>Ctrl+Alt+Del</strong> only when the selected guest is expected to receive that input.</li></List><p>The <strong>Ignore TLS certificate errors</strong> option is disabled by default. Enable it only for a trusted private Proxmox endpoint whose certificate cannot be installed in the operating system trust store.</p><p>Connection Controls stay on screen next to the VNC display at all times -- select <strong>Collapse</strong> to give the VNC screen most of the available height, or <strong>Expand</strong> to see the Node/VM pickers and Connect/Disconnect/Logout buttons again. Collapsing or expanding never disconnects the session.</p></Page> },
      { id: "proxmox-file-transfer", title: "File transfer credentials and upload/download", summary: "Move files into and out of the connected VM.", content: <Page><p>Once a VNC session connects, nFterm automatically looks for a way to reach the guest for file transfer, in this order:</p><List><li><strong>Direct SFTP</strong> -- the VM's own IP is reachable from this computer on its SSH port.</li><li><strong>SFTP via host jump</strong> -- the VM isn't directly reachable, but the Proxmox host is, and file transfer is tunneled through it.</li><li><strong>Guest Agent (limited)</strong> -- neither is reachable; falls back to the Proxmox QEMU Guest Agent (QEMU guests only, no folder download, uploads are size-limited).</li></List><p>The detected mode and the guest IP it's using are shown as a status pill above the file list. If none of the three succeed, an explanation is shown instead of the file list -- check the VM's network/Guest Agent status, or configure SSH credentials and a fallback IP as described below.</p><h3>Setting up VM SSH / Host SSH (jump) credentials</h3><p>Open the Proxmox entry for <strong>Edit</strong> and select the <strong>VM SSH</strong> or <strong>Host SSH (jump)</strong> button under the title:</p><List><li><strong>VM SSH</strong>: the VM's own SSH username, port, optional private key path, and password -- used for Direct SFTP and as the tunneled destination for SFTP via host jump. A <strong>Fallback VM IP</strong> can be set here for guests the Guest Agent can't report an address for (typically LXC).</li><li><strong>Host SSH (jump)</strong>: the Proxmox host's own SSH username, port, optional private key path, and password -- only needed when the VM isn't directly reachable and file transfer must tunnel through the host.</li></List><p>Each page has its own <strong>Install SSH key</strong> button, which pushes nFterm's managed public key onto that account's <code>authorized_keys</code> using the password saved on the same page. Leave a password field blank when editing an existing entry to keep the previously saved password; passwords are kept in the desktop credential store, never in workspace JSON.</p><h3>Browsing and transferring files</h3><p>Once a route is found, the left sidebar (where the Proxmox entries list normally is) switches to the VM's remote file browser:</p><List><li>Use <strong>Upload</strong> to pick one or more local files to send into the current remote folder.</li><li>Select files with the row checkboxes (folders can also be selected, except under Guest Agent mode, which has no folder download support), then use <strong>Download</strong> to choose a local destination folder.</li><li>Use <strong>Refresh</strong> to reload the current folder's listing.</li><li>Click a folder's name, or the <strong>.. (up)</strong> row, to navigate; the path above the table shows where you are.</li><li>Active and finished transfers appear in a queue below the file list, with automatic retry for recoverable errors; select <strong>Remove</strong> to clear a completed or failed entry.</li><li>Select <strong>&larr; Entries</strong> to return to the Proxmox entries list -- this does not disconnect the VNC session or log out of the Proxmox web session.</li></List></Page> },
      { id: "proxmox-troubleshooting", title: "Proxmox troubleshooting", summary: "Diagnose login, VM discovery, and VNC failures.", content: <Page><List><li>Confirm the base URL uses <code>https://</code>, the API port is reachable, and the host certificate is trusted unless the explicit TLS option is enabled.</li><li>Confirm the account has permission to list cluster resources and open a VNC proxy for the selected guest.</li><li>Confirm the selected node, VMID, and guest type match the current cluster.</li><li>If a connection is switched or cancelled while starting, select the entry again and start a new connection; pending relay state is intentionally discarded.</li><li>For server-side failures, inspect the non-sensitive operation log entry and the Proxmox server task log.</li><li>If the file browser reports no reachable file-transfer route, confirm the QEMU Guest Agent is installed and running (QEMU guests), or add VM SSH / Host SSH (jump) credentials and a fallback VM IP on the entry's <strong>VM SSH</strong> / <strong>Host SSH (jump)</strong> pages.</li></List></Page> },
    ],
  },
  {
  ...
  },
  {
    id: "logs-history", title: "Logs and History", icon: "history", pages: [
      ...
      { id: "operation-log", title: "Operation logs", summary: "Inspect and export the audit trail.", content: <Page><p>Operation logs are JSON-lines records for file, SSH, transfer, and application events. They exclude secrets, rotate at 10 MB, and retain at most three files. Settings displays the active log path and retained files.</p><h3>REST API mode</h3><p>REST API mode supports HPE iLO 5, iLO 6, and iLO 7 Redfish services. IML Monitor operation logs and status messages do not include passwords, tokens, cookies, or authorization headers.</p><p>The REST endpoint is always taken from the selected REST entry. Enter the complete scheme and host in the entry Base URL; the tool does not use a fixed hardware URL.</p><p>OpenBMC authentication follows the official OpenBMC REST login flow at <code>/login</code>, which returns a token sent as <code>X-Auth-Token</code>. Redfish SessionService is used for HPE and generic Redfish profiles when advertised.</p><p>References: <a href="https://servermanagementportal.ext.hpe.com/docs/redfishservices/ilos/ilo6/" rel="noreferrer">HPE iLO Redfish documentation</a> and <a href="https://github.com/openbmc/docs/blob/master/REST-cheatsheet.md" rel="noreferrer">OpenBMC REST cheat sheet</a>.</p></Page> },
    ],
  },
  {
