...
type Location = { id: string; displayName: string; status?: string; readOnly?: boolean; capabilities?: string[] };
type Session = { host: string; port: string; token: string; username: string; userId: number | null; role: string; permissions: string[]; locationId: string; ignoreTlsErrors: boolean };
...
const appVersion = import.meta.env.VITE_APP_VERSION_DISPLAY || import.meta.env.VITE_APP_VERSION || "";
const initialSession: Session = { host: defaultHost, port: defaultPort, token: "", username: "", userId: null, role: "", permissions: [], locationId: "", ignoreTlsErrors: false };
...
function App() {
  ...
  const activeLocation = locations.find(location => location.id === session.locationId);
  const hasCapability = (capability: string) => activeLocation?.capabilities?.includes(capability) === true;
  ...
  const loadLocations = async () => {
    ...
    try {
      ...
      const available = (data.locations || []).filter(location => location.id);
      setLocations(available);
      setSession(current => ({ ...current, locationId: available.some(location => location.id === current.locationId) ? current.locationId : available[0]?.id || "" }));
    } finally {
  ...
  useEffect(() => {
    if (session.token) {
      void loadLocations().catch(error => setNotice(error instanceof Error ? error.message : String(error)));
      const healthTimer = window.setInterval(() => void loadLocations().catch(error => setNotice(error instanceof Error ? error.message : String(error))), 15000);
  ...
  const download = () => run(async () => {
    ...
    notify(singleFile ? `Preparing download: ${fileName}...` : `Preparing archive for ${selectedItems.length} item${selectedItems.length === 1 ? "" : "s"}...`, 0);
    const destination = await invoke<string>("download_to_disk", {
      url: singleFile ? `${serverUrl(session)}/api/files/download/${downloadPath(selectedItems[0].path)}` : `${serverUrl(session)}/api/archive`,
      method: singleFile ? "GET" : "POST",
       headers: session.token ? [["Authorization", `Bearer ${session.token}`], ...(session.locationId ? [["X-Location-ID", session.locationId]] : []), ...(singleFile ? [] : [["Content-Type", "application/json"]])] : [],
       body: singleFile ? undefined : Array.from(new TextEncoder().encode(JSON.stringify({ items: selectedItems.map(({ name, isDirectory, path: itemPath }) => ({ name, isDirectory, path: itemPath })), currentPath: path, locationId: session.locationId }))),
      fileName,
      ignoreTlsErrors: session.ignoreTlsErrors
    });
  ...
  const uploadPaths = (paths: string[]) => void run(async () => {
    ...
    if (!accepted) return;
    const headers = session.token ? [["Authorization", `Bearer ${session.token}`], ...(session.locationId ? [["X-Location-ID", session.locationId]] : [])] : [];
    notify(`Uploading ${summary.files} file${summary.files === 1 ? "" : "s"}...`, 0);
    ...
    if (!batchId) {
      notify(`Uploaded ${summary.directories} folder${summary.directories === 1 ? "" : "s"}.`);
      await loadFiles(path);
      return;
    }
    for (let attempt = 0; attempt < 600; attempt += 1) {
      const progress = await api(`/api/progress/batch/${encodeURIComponent(batchId)}`);
      ...
      notify(`Uploading: ${batch.successCount}/${batch.totalFiles} files (${Math.round(batch.progress)}%)`, 0);
      if (batch.status === "completed") {
        notify(`Uploaded ${batch.successCount} file${batch.successCount === 1 ? "" : "s"}.`);
        await loadFiles(path);
        return;
      }
      if (batch.status === "failed" || batch.status === "partial_fail") throw new Error(`Upload finished with ${batch.failedCount} failed file${batch.failedCount === 1 ? "" : "s"}.`);
  ...
  const share = () => run(async () => {
    if (selectedItems.length !== 1 || selectedItems[0].isDirectory) return;
     const response = await api("/api/files/share", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ locationId: session.locationId, filePath: selectedItems[0].path }) });
    if (!response.ok) throw new Error(await readError(response));
  ...
  if (!session.token) return <main className="login"><form onSubmit={login}><div className="login-mark" aria-hidden="true"><span /></div><h1>File Transfer</h1><p>Sign in to your file server over HTTPS.</p><label>Server address<input placeholder="files.example.internal" value={session.host} onChange={event => setSession(current => ({ ...current, host: event.target.value }))} /></label><label>HTTPS port<input inputMode="numeric" value={session.port} onChange={event => setSession(current => ({ ...current, port: event.target.value }))} /></label><label>Username<input value={session.username} onChange={event => setSession(current => ({ ...current, username: event.target.value }))} /></label><label>Password<input type="password" value={password} onChange={event => setPassword(event.target.value)} /></label><label className="tls-option"><input type="checkbox" checked={session.ignoreTlsErrors} onChange={event => setSession(current => ({ ...current, ignoreTlsErrors: event.target.checked }))} /> Ignore SSL certificate verification <small>Only enable for a server you trust.</small></label><button disabled={busy}>{busy ? "Signing in..." : "Sign in"}</button>{notice && <output role="alert">{notice}</output>}{appVersion && <small className="login-version">{appVersion}</small>}</form></main>;

  return <main className="explorer">
    <header className="titlebar"><span className="app-mark" /><span className="app-name">LAB File Manager</span><span className="connection-status">SECURE STORAGE</span>{activeLocation && <div className="location-control"><span className="location-label">Location</span>{locations.length > 1 ? <select className="location-select" aria-label="Location" value={session.locationId} disabled={busy || locationsLoading} onChange={event => selectLocation(event.target.value)}>{locations.map(location => <option key={location.id} value={location.id}>{location.displayName}</option>)}</select> : <span className="location-single">{activeLocation.displayName}</span>}<span className={`health-dot ${activeLocation.status === "online" ? "online" : ""}`} title={activeLocation.status || "unknown"} aria-label={activeLocation.status || "unknown"} /></div>}<div className="account-control" ref={accountControl}><button className="account" onClick={event => { event.stopPropagation(); setAccountOpen(open => !open); }} aria-expanded={accountOpen}>{session.username}<span className="account-role">{session.role === "admin" ? "Admin" : "User"}</span><span className="account-chevron">⌄</span></button>{accountOpen && <div className="account-menu"><div className="account-summary"><strong>{session.username}</strong><span>{session.role === "admin" ? "System administrator" : "Standard user"}</span></div><button onClick={() => { setAccountOpen(false); setChangePasswordOpen(true); }}>Change password</button><hr /><button className="danger" onClick={signOut}>Log out</button></div>}</div></header>
      <nav className="commandbar" aria-label="File actions"><button className="primary" onClick={upload} disabled={busy || !locationOnline || !hasCapability("upload")}>Upload</button><button onClick={createFolder} disabled={busy || !locationOnline || !hasCapability("mkdir")}>New folder</button><span className="divider" /><button disabled={busy || !locationOnline || !selectedItems.length || !hasCapability("read")} onClick={download}>Download</button><button disabled={busy || !locationOnline || !selectedItems.length || !hasCapability("move")} onClick={() => notify("Drag selected files to a destination folder to move them.")}>Move</button><button disabled={busy || !locationOnline || selectedItems.length !== 1 || !hasCapability("rename")} onClick={rename}>Rename</button><button disabled={busy || !locationOnline || selectedItems.length !== 1 || selectedItems[0].isDirectory || !hasCapability("share")} onClick={share}>Share</button><button disabled={busy || !locationOnline || !selectedItems.length || !hasCapability("delete")} onClick={remove}>Delete</button><span className="divider" /><button onClick={() => setSelected(selected.length === files.length ? [] : files.map(file => file.path))}>Select all</button><span className="view-switch"><button className={viewMode === "details" ? "active" : ""} onClick={() => setViewMode("details")}>Details</button><button className={viewMode === "grid" ? "active" : ""} onClick={() => setViewMode("grid")}>Grid</button></span><button onClick={() => { void loadLocations(); void run(() => loadFiles(path)); }} disabled={busy}>Refresh</button></nav>
    ...
    <div className="desktop-workspace"><section className="desktop-content"><div className="content-heading"><div><span className="eyebrow">CURRENT DIRECTORY</span><h1>{searching ? `Search results for "${search}"` : path || "/"}</h1></div>{selectedItems.length > 0 && <span className="selection-count">{selectedItems.length} selected</span>}</div>{notice && <output className="notice transfer-notice" role="status">{notice}</output>}{shareUrl && <div className="share-link"><label>Share link<input readOnly value={shareUrl} onFocus={event => event.currentTarget.select()} /></label><button onClick={() => void navigator.clipboard.writeText(shareUrl).then(() => notify("Share link copied.")).catch(error => setNotice(error.message))}>Copy link</button><button onClick={() => setShareUrl("")}>Close</button></div>}<div className="file-area">{viewMode === "grid" ? <div className="file-grid">{files.map(file => <article key={file.path} className={`file-tile ${selected.includes(file.path) ? "selected" : ""} ${dropTarget === file.path ? "drop-target" : ""}`} draggable onDragStart={event => beginDrag(event, file)} onDragEnd={finishDrag} onDragOver={event => { if (file.isDirectory && isValidMoveTarget(dragItems, file.path)) { event.preventDefault(); setDropTarget(file.path); } }} onDrop={event => { if (file.isDirectory) { event.preventDefault(); const items = dragItems; finishDrag(); void moveItems(items, file.path); } }} onClick={event => { if (event.ctrlKey || event.metaKey) toggle(file, true); else setSelected([file.path]); }} onDoubleClick={() => file.isDirectory ? void run(() => loadFiles(file.path)) : download()} onContextMenu={event => { event.preventDefault(); if (!selected.includes(file.path)) setSelected([file.path]); setContextMenu({ x: event.clientX, y: event.clientY }); }}><span className="tile-icon">{file.isDirectory ? "📁" : "📄"}</span><strong>{file.name}</strong><span>{file.isDirectory ? "File folder" : "File"}</span><small>{file.isDirectory ? "Drop files here" : formatSize(file.size)}</small></article>)}</div> : <table className="file-table"><thead><tr><th aria-label="Select" /><th>Name</th><th>Modified</th><th>Size</th></tr></thead><tbody>{files.map(file => <tr key={file.path} draggable className={`file-row ${selected.includes(file.path) ? "selected" : ""} ${dropTarget === file.path ? "drop-target" : ""}`} onDragStart={event => beginDrag(event, file)} onDragEnd={finishDrag} onDragOver={event => { if (file.isDirectory && isValidMoveTarget(dragItems, file.path)) { event.preventDefault(); setDropTarget(file.path); } }} onDrop={event => { if (file.isDirectory) { event.preventDefault(); const items = dragItems; finishDrag(); void moveItems(items, file.path); } }} onClick={event => { if (event.ctrlKey || event.metaKey) toggle(file, true); else setSelected([file.path]); }} onDoubleClick={() => file.isDirectory ? void run(() => loadFiles(file.path)) : download()} onContextMenu={event => { event.preventDefault(); if (!selected.includes(file.path)) setSelected([file.path]); setContextMenu({ x: event.clientX, y: event.clientY }); }}><td><input aria-label={`Select ${file.name}`} type="checkbox" checked={selected.includes(file.path)} onChange={event => toggle(file, event.target.checked)} onClick={event => event.stopPropagation()} /></td><td><span className="file-name">{file.isDirectory ? "📁" : "📄"} {file.name}</span></td><td className="muted">{file.modified ? new Date(file.modified).toLocaleString() : "--"}</td><td className="muted">{file.isDirectory ? "--" : formatSize(file.size)}</td></tr>)}</tbody></table>}</div></section><aside className="sidebar desktop-folder-tree"><span className="sidebar-label">Locations</span>{renderTreeNode(folderTree)}</aside></div>
    ...
    {contextMenu && <div className="context-menu" style={{ left: contextMenu.x, top: contextMenu.y }} onClick={event => event.stopPropagation()}><button disabled={!selectedItems.length} onClick={() => { setContextMenu(null); download(); }}>Download</button><button disabled={!selectedItems.length} onClick={() => { setContextMenu(null); notify("Drag selected files to a destination folder to move them."); }}>Move</button><button disabled={selectedItems.length !== 1} onClick={() => { setContextMenu(null); rename(); }}>Rename</button><button disabled={selectedItems.length !== 1 || selectedItems[0].isDirectory} onClick={() => { setContextMenu(null); share(); }}>Share</button><hr /><button disabled={!selectedItems.length} onClick={() => { setContextMenu(null); remove(); }}>Delete</button></div>}
