...
A left-button selection is copied after xterm.js completes its selection, even when the pointer is released outside the terminal host. Temporary document-level mouseup tracking belongs to the tab that started the selection and is cancelled on right-button input, tab changes, blur, pointer cancellation, close, and unmount, so it cannot overwrite the clipboard from an old tab. Selecting an SSH tab focuses its xterm after activation so typing can begin without a second click.
